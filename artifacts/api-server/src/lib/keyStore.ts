...
export async function addKey(
  ...
  await db.insert(keysTable).values({ key, expiresAt, ip, hwid }).onConflictDoNothing();
}
...
export async function findValidKeyByIp(ip: string): Promise<typeof keysTable.$inferSelect | null> {
  const rows = await db
    .select()
    .from(keysTable)
    .where(and(eq(keysTable.ip, ip), isNotNull(keysTable.expiresAt), gt(keysTable.expiresAt, new Date())))
    .limit(1);
  return rows[0] ?? null;
}

export async function findValidKeyByHwid(hwid: string): Promise<typeof keysTable.$inferSelect | null> {
  const rows = await db
    .select()
    .from(keysTable)
    .where(and(eq(keysTable.hwid, hwid), isNotNull(keysTable.expiresAt), gt(keysTable.expiresAt, new Date())))
    .limit(1);
  return rows[0] ?? null;
}
